...
const MenuIcon = forwardRef<MenuIconHandle, MenuIconProps>(
    ({ className, size = 28, isOpen = false, onOpenChange, ...props }, ref) => {
        ...
        return (
            <button
                className={cn(
                    `cursor-pointer select-none p-2 hover:bg-secondary rounded-md transition-colors duration-200 flex items-center justify-center`,
                    className,
                )}
                onClick={handleClick}
                {...props}
